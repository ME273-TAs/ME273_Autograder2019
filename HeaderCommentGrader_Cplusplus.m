...
% try                             % TRY structure used to catch errors
    ...
    while i <= length(content)      % loop through the lines of "contents"
        ...
        if length(content{i}) > 2       % check that each line has at least 3 characters (otherwise it will be ignored)
            ...
            if length(comdex)>0 && content{i}(comdex+1) == '/'  % if comdex is nonempty it will have a length > 0, and thus no comment character was found
...
if ignorecase
    ...
    for i = 1:length(bigString)-length(phrase)
        if length(phrase) ~= length(bigString(i:i-1+length(phrase)))
            'pause'
        end
        if strcmp(phrase,bigString(i:length(phrase)))
            result = 1;
            return
        end
    end
    ...
    for i = 1:length(bigString)-length(phrase)
        if strcmp(phrase,bigString(i:i+length(phrase)))
            result = 1;
            return
        end
    end

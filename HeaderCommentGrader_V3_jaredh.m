function [HeaderScore, HeaderFeedback, CommentScore, CommentFeedback, error] = HeaderCommentGrader_V3_jaredh(filename)
%--------------------------------------------------------------
...
try                             % TRY structure used to catch errors
    ...
    while i <= length(content)      % loop through the lines of "contents"
                ...
                while header == 1                       % loop through header rows
                    ...
                    if length(content{i}) > 2           % check that the length of this line is at least 3 characters long
                        ...
                        if (contains(content{i},'END','IgnoreCase',true) && contains(content{i},'HEADER','IgnoreCase',true))    % if we encounter a line containing 'END' and 'HEADER', we are leaving the header section
                            header = -1;                         % set header indicator to 0 - OFF
                            i = i + 1;                          % increment i
                            break                               % bail out
                        else
        ...
        % BODY SECTION
        if length(content{i}) > 2 && localFunction == 0  % check that each line has at least 3 characters (otherwise it will be ignored)
        ...
        if header == -1 && contains(content{i},'end','IgnoreCase',true)
            for lf = 1:5
                try
                    if length(content{i}) > 2
                        if contains(content{i+lf},'function','IgnoreCase',true)
                            localFunction = 1;
                        end
                    else
                        break
